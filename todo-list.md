...
#### ✅ タスク 11-3: バックエンド直接接続の設定 (Dev環境先行)

フロントエンドからの動画アップロードを、BFFを経由せずバックエンドAPIへ直接送信するように変更する。
現状、BFFルートが存在せず、アップロードが失敗するため。

**サブタスク**:

- [x] **11-3a: クライアントコード修正**
  - [x] `video-uploader.client.ts`: 接続先を `APP_CONFIG.API.ENDPOINTS.UPLOAD` に変更
- [x] **11-3b: 環境変数修正 (Dev)**
  - [x] `wrangler.jsonc`: Dev/Prod環境の `VITE_API_BASE_URL` をカスタムドメインに変更
- [x] **11-3c: 動作確認**
...
- [x] **11-4b: 動作確認**
  - [x] GitHub Actions での実行確認

---

### 🐛 **フェーズ 11.5: バグフィックス**

#### ✅ タスク 11.5-1: ドラッグ＆ドロップ時にブラウザがファイルを開く問題

- [x] **11.5-1a**: `entry.client.tsx` でグローバルD&Dイベント無効化（Hydration前に登録）
- [x] **11.5-1b**: `root.tsx` から重複コードを削除
- [x] **11.5-1c**: `index.html` の削除（SSRでは不要、Viteが優先して返していた）
- [x] **11.5-1d**: `.gitignore` / `.prettierignore` の更新

---

### 🐛 **フェーズ 12: ファイルアップロードUXの改善とバグ修正**

#### ✅ タスク 12-1: ファイル選択ボタンの挙動修正

- [x] `UploadDropzone` のクリックイベント監視修正
- [x] `react-dropzone` から自前 `input` 実装への移行（確実なFinder起動のため）
- [x] バリデーション（サイズ、形式）の自前実装
- [x] ユニットテストの修正

#### ✅ タスク 12-2: APIタイムアウト設定と処理中ローディングUIの改善

- [x] `VITE_API_TIMEOUT` を 360000 (6分) に変更
- [x] 処理中のローディングオーバーレイ実装（暗い背景 + 中央スピナー）
- [x] 処理進捗の表示（アップロード中 → 処理中 → 完了）
...
- ✅ Cloudflare Workers: ビルドインフラが Bun をサポート（デフォルト v1.2.15）
- ✅ Vite / React Router v7: 公式サポートあり、互換性問題なし
- ✅ wrangler / Vitest: Bun から実行可能
- ✅ ソースコード: **変更不要**
