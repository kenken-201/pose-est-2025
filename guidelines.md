...
デスクトップ向け Web アプリケーション (`kenken-pose-est.com`) において、React Router v7（BFF）を経由して姿勢推定バックエンド API に動画を送信し、処理結果をブラウザで表示するシステムを構築します。
...
2.  **細粒度な実装サイクル (One by One)**:
    ...
    - 実装後、**必ず** `npm run test:coverage` (必要に応じて `lint`) を実行し、動作確認を行います。
